...
public class WorkerService {
    ...
    public WorkerService(AssistantRepository assistantRepository,
    ...
    public List<AssistantOverviewDTO> getAllAssistants() {
        ...
        return allAssistants.stream().map(worker -> {
            List<WorkTime> assistantWorkTimeList = workTimeRepository.findAllByAssistantId(worker.getId());

            Duration worktimeThisMonth = assistantWorkTimeList.stream()
                    .filter(time -> time.getStartTime().getMonth().equals(LocalDateTime.now().getMonth()))
                    .map(WorkTime::getTotalTime).reduce(Duration.ZERO, Duration::plus);
            String timeInHHMMtM = convertDurationToReadableTimeString(worktimeThisMonth);

            Duration worktimeLastMonth = assistantWorkTimeList.stream()
                    .filter(time -> time.getStartTime().getMonth().equals(LocalDateTime.now().getMonth().minus(1)))
                    .map(WorkTime::getTotalTime).reduce(Duration.ZERO, Duration::plus);
            String timeInHHMMlM = convertDurationToReadableTimeString(worktimeLastMonth);

            return new AssistantOverviewDTO(worker.getId(), worker.getName(), timeInHHMMtM, timeInHHMMlM);
        }).toList();
    }
    ...
    public List<WorkTimeAssistantDTO> getAllWorkTime() {
        ...
        return workTimeList.stream().map(workTime -> {

            Optional<Assistant> assistant = assistantList.stream()
                    .filter(co -> co.getId().equals(workTime.getAssistantId())).findAny();
            ...
            return new WorkTimeAssistantDTO(
                    ...
                    workTime.getInfo());
        }).sorted(Comparator.comparing(WorkTimeAssistantDTO::startTime).reversed()).toList();
    }
    ...
    public List<WorkTimeAssistantDTO> getAllWorkTimeById(UUID assistantId) {
        ...
        return workTimeList.stream().map(time -> new WorkTimeAssistantDTO(time.getId(),
                time.getAssistantId(), assistantName, time.getStartTime(), time.getEndTime(),
                convertDurationToReadableTimeString(Duration.between(time.getStartTime(), time.getEndTime())),
                time.getInfo())).sorted(Comparator.comparing(WorkTimeAssistantDTO::startTime).reversed()).toList();
    }
    ...
    public AssistantTotalOverviewDTO getAllWorkTimeOverviewById(UUID assistantId) {
        ...
        Duration workDuration = workTimes.stream().map(time -> Duration.between(time.getStartTime(), time.getEndTime())).reduce(Duration::plus).orElse(Duration.ofMinutes(0));

        return new AssistantTotalOverviewDTO(assistantId, assistantName, convertDurationToReadableTimeString(workDuration));
    }
    ...
    public ResponseEntity<String> logPartTimeWorkTime(String nfcId) {
        ...
        Optional<WorkTime> workTimeToday = workTimeList.stream().filter(WorkTime::isWorkingStatus)
                .findAny();

        if (workTimeToday.isPresent() && workTimeToday.get().isWorkingStatus() && workTimeToday.get().getStartTime().getDayOfMonth() == LocalDateTime.now().getDayOfMonth()) {
            workTimeToday.get().setEndTime(LocalDateTime.now());
            workTimeRepository.save(workTimeToday.get());

            return new ResponseEntity<>("ausgeloggt, " + assistant.getName() + ". Deine heutige Arbeitszeit beträgt " + convertWorkingToUnderstandableString(workTimeToday.get().getTotalTime()), HttpStatusCode.valueOf(201));
        } else {
            ...
            return new ResponseEntity<>("eingeloggt, " + assistant.getName(), HttpStatusCode.valueOf(200));
        }
    }
    ...
    public void addNewUserToDatabase(AddUserDTO addUserDTO) {
        ...
        customUserRepository.save(new CustomUser(addUserDTO.username(),bCryptPasswordEncoder.encode(addUserDTO.password()),addUserDTO.role()));
    }
